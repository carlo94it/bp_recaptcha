<?php
/**
* This plugin is based on reCaptcha PHP library powered by reCaptcha.net
* but rewrited using Bokeh Platform code style
*
* ORIGINAL AUTHORS:
*   Mike Crawford
*   Ben Maurer
...
class plugin_recaptcha
{
	...
	*/
	function recaptcha_check_answer($remoteip, $challenge, $response, $extra_params = array())
	{
		...
		$response = $this->_recaptcha_http_post(RECAPTCHA_VERIFY_SERVER, "/recaptcha/api/verify", array('privatekey' => $this->private_key, 'remoteip' => $remoteip, 'challenge' => $challenge, 'response' => $response) + $extra_params);
	...
	*/
	function _recaptcha_aes_encrypt($val, $ky)
	{
		...
		{
			error_box("[reCaptcha] To use reCAPTCHA Mailhide, you need to have the mcrypt php module installed.");
		}
	...
	*/
	function recaptcha_mailhide_url($email)
	{
		...
		return "http://www.google.com/recaptcha/mailhide/d?k=" . $this->public_key . "&c=" . $this->_recaptcha_mailhide_urlbase64($cryptmail);
	}
